...
member_tickers = members["ticker_at_date"].tolist()
...
# Score EXACTLY the survivorship-free universe we just reconstructed — pass the
# members explicitly so the factor set matches the membership set (and includes
# names that were members on AS_OF_DATE but have since been delisted/acquired).
# `as_of` is belt-and-suspenders: the client view is already PIT-filtered, and
# the engine *also* injects `AND filing_date <= as_of` into each factor SQL.
result = engine.compute(universe=member_tickers, as_of=AS_OF_DATE, min_factors=3)
...
#
# `combine()` returns one score per company IN result.df row order, so assign it
# positionally (`.to_numpy()`). Do NOT `.join()` on `symbol` — symbols are not
# guaranteed unique (share classes, recycled tickers), and an index join on a
# duplicated key fans out into a cartesian product.
dataset = result.df.copy()
dataset["composite_alpha"] = result.rank().combine().to_numpy()
dataset = dataset.set_index("symbol").sort_values("composite_alpha", ascending=False)
...
pit_sql = f"""
    ...
    -- The single latest 10-K KNOWN by the cutoff, per member. We deliberately do
    -- NOT hand-derive a ratio here: per Valuein's provenance discipline a metric
    -- should come pre-computed (with its inputs) from the engine, never from
    -- arithmetic on raw facts. The job of this block is to make the PIT +
    -- survivorship mechanics auditable — that filing_date <= as_of gates what
    -- was knowable, and report_date (the period end) trails it.
    SELECT m.symbol, m.name, m.sector,
           f.filing_date, f.report_date,
           DATE_DIFF('day', f.report_date, f.filing_date) AS pit_lag_days
    FROM members m
...
if not proof.empty:
    # Every filing_date is <= as_of, while report_date (the period end) pre-dates
    # it — `pit_lag_days` IS the look-ahead gap PIT discipline closes: the numbers
    # existed for the period end, but weren't KNOWABLE to the market until filed.
    print(
        proof[["symbol", "filing_date", "report_date", "pit_lag_days"]]
        .head(8)
...
# On the Institutional/`full` tier, reconstruct the full survivorship-free
# universe (e.g. RUSSELL3000, or every active+delisted CIK) and pass those
# tickers to `compute(universe=...)` the same way. For fact-heavy work, use
# client.stream(sql) so the `fact` table is never fully materialised in RAM:
