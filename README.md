...
| `index_membership` | Index constituents keyed on `cik`, with `effective_date` / `removal_date` half-open windows. **SP500 is survivorship-free back to 1996** — real entry/exit spells including long-delisted registrants. **RUSSELL1000 / RUSSELL2000 / RUSSELL3000 are current-composition only** — one open spell per member at the latest snapshot, no removals, no prior history. | Reconstruct the **S&P 500** on any historical date. A past-dated query returns nothing for the three Russell indices. JOIN `references.cik = index_membership.cik` for company metadata. |
