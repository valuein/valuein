...
| `index_membership` | Index constituents keyed on `cik`, with `effective_date` / `removal_date` half-open windows. **SP500 is survivorship-free back to 1996** (`confidence='high'`) — real entry/exit spells including long-delisted registrants. **RUSSELL1000 / RUSSELL2000 / RUSSELL3000 go back to 2007-01-31** (`confidence='medium'`, `source='ishares_history'`) — derived from the *monthly* disclosed holdings of the iShares ETFs that track each index, so dates are accurate to within a month and the fund proxies rather than defines the index. Graded against FTSE Russell's published reconstitution lists: **recall 96.5–98.3%, precision 93.6–98.6%**. No Russell data before 2007, and none for 2017-01→2017-06 (the fund provider publishes no holdings file in that window). | Reconstruct the **S&P 500** or a **Russell** index on a historical date. Branch on `confidence` — `medium` means approximate, not authoritative. JOIN `references.cik = index_membership.cik` for company metadata. |
